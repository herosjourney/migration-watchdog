...
class Finding:
    ...
    scan_timestamp: str = ""
    status: str = "pending"  # "pending", "approved", "declined", "superseded"
    ...
    auditor_payload: dict | None = None  # structured payload from currency/automation auditors
    finding_schema_version: str | None = None  # e.g. "currency/1.0", "automation/1.0"
...
class ScanRun:
    ...
    trigger_type: str = "scheduled"  # "scheduled" | "pull_request"
    audited_files: list[str] = field(default_factory=list)  # files scanned (empty = all)
    partial_data_warning: bool = False  # True when some sources failed to load
    source_pr_number: int | None = None  # PR number when trigger_type="pull_request"
    source_pr_head_sha: str | None = None  # HEAD SHA of the triggering PR
    source_pr_html_url: str | None = None  # HTML URL of the triggering PR
