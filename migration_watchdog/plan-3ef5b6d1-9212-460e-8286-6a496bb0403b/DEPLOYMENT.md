...
| `WATCHDOG_PACKAGE_SUBDIR` | Variable | Inner package directory name (auto-detected if not set; set explicitly if layout changes) |
...
```bash
# From the watchdog repo root
ln -sf <UUID-dir> migration_watchdog  # create symlink if not present
pip install -e .

# Run tests in the correct order (stub isolation)
