...
class FindingsRepository:
    ...
    def list_findings(
        ...
        - run_id  -> main table PK=RUN#{run_id}, SK begins_with FINDING#
          (uses the FindingByRun sparse projection written by save_finding)
        - exclude_dismissed -> filter out findings with active dismissal cooldowns

        Note: the dashboard's /api/findings?run_id= endpoint uses a separate
        full-table scan with FilterExpression on run_id rather than this method.
        Both approaches are correct for v1; the GSI-based path here is faster
        for large tables. Consider unifying to a single mechanism in v2.
        """
    ...
    def _item_to_finding(item: dict) -> Finding:
        ...
        return Finding(
            ...
            dismissal=dismissal,
            auditor_payload=item.get("auditor_payload"),  # backward-compatible: None for old items
            finding_schema_version=item.get("finding_schema_version"),  # backward-compatible: None for old items
        )
    ...
    def _run_to_item(run: ScanRun) -> dict:
        ...
        return {
            ...
            "audited_files": run.audited_files if run.audited_files else [],
            "partial_data_warning": run.partial_data_warning,
            "source_pr_number": run.source_pr_number,
            "source_pr_head_sha": run.source_pr_head_sha,
            "source_pr_html_url": run.source_pr_html_url,
        }
    ...
    def _item_to_run(item: dict) -> ScanRun:
        ...
        return ScanRun(
            ...
            audited_files=list(item.get("audited_files") or []),
            partial_data_warning=item.get("partial_data_warning", False),
            source_pr_number=item.get("source_pr_number"),
            source_pr_head_sha=item.get("source_pr_head_sha"),
            source_pr_html_url=item.get("source_pr_html_url"),
        )
