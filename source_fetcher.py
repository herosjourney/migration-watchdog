...
class AwsDocsSearcher:
    ...
    async def search(self, query: str, limit: int = 3) -> list[dict]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=self._timeout) as client:
                resp = await client.get(self._SEARCH_API, params=params)
    ...
    def search_and_fetch_sync(self, query: str) -> str:
        ...
        loop.run_until_complete() when one is already running
        (e.g., in Jupyter or certain test runners).
        """
        try:
        ...
        if loop is None:
            return asyncio.run(self.search_and_fetch(query))
        else:
            return loop.run_until_complete(self.search_and_fetch(query))
